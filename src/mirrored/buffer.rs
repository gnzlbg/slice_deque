...
impl<T> Buffer<T> {
    ...
    fn size_in_bytes(len: usize) -> usize {
        no_required_allocation_units(len * ::std::mem::size_of::<T>()) * allocation_granularity()
    }
    ...
    pub unsafe fn uninitialized(len: usize) -> Result<Self, ()> {
        // Zero-sized types are not supported yet:
        assert!(::std::mem::size_of::<T>() > 0);
        // The alignment requirements of `T` must be smaller than the allocation
        // granularity.
    ...
    /// * 3. Race condition: mirror bytes of the first half into the second half.
    ///
    ...
    ///   bytes (by allocating memory with `VirtualAlloc` and immediately freeing
    ///   it with `VirtualFree`).
    ...
    /// memory region where we wanted to map our physical memory) we release the
    /// first portion of virtual memory if mapping succeeded and try again (up
    /// to a maximum of `MAX_NO_ALLOC_ITERS` times).
    #[cfg(target_os = "windows")]
    unsafe fn allocate_uninitialized(alloc_size: usize) -> Result<Self, ()> {
        let half_alloc_size = alloc_size / 2;
        ...
        let virt_ptr = loop {
            ...
            if let Err(_) = map_file_to_memory(file_mapping, half_alloc_size, virt_ptr) {
            ...
            if let Err(_) = map_file_to_memory(file_mapping, half_alloc_size,
                           virt_ptr.offset(half_alloc_size)) {
                // If this fails, we release the map of the first half and try
                // again:
                no_iters += 1;
                if let Err(_) = unmap_file_mapped_memory(virt_ptr) {
...
impl<T> Drop for Buffer<T> {
    ...
    fn drop(&mut self) {
        ...
        unmap_file_mapped_memory(self.ptr)
            .expect("unmapping first buffer half failed");
        unmap_file_mapped_memory(self.ptr.offset(half_alloc_size))
            .expect("unmapping second buffer half failed");
        close_file_mapping(self.file_mapping)
            .expect("freeing physical memory failed");
    }
}
...
mod tests {
    ...
    #[test]
    fn allocations() {
        let elements_per_alloc_unit = allocation_granularity() / ::std::mem::size_of::<u64>();
        let sizes = [
            8,
